...
<main class="main-histoire">
    ...
    <?php
        ...
        echo '<div class="flex"><section class="form_destination">';
        
        //Get choice is the parent on contexte
        
        

        $idHistory = $_GET['idhistory'];    
        //Get all choice associate a this contexte
        $query = $dbCo->prepare("SELECT c.text_button, c.id_choice FROM choice c JOIN node n ON c.id_node_parent = n.id_node
        JOIN (
            SELECT id_node
            FROM node 
            JOIN choice  ON id_node_parent = id_node_parent
            WHERE id_history = :id
        ) parent ON id_node_parent = parent.id_node
